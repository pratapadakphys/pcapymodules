...
from .. import lanmpprojectv4 as lp
import pandas as pd
import os
from .spereaderv5  import *
TAG = True
...
class SpeFileSet(FileSet):
    def __init__(self, files, pattern = None, filename = None):
        super().__init__(files, pattern, filename)
    ...
    def data(self, variable = None):
        ...
        dt = pd.DataFrame(np.stack(self.df['y']).transpose(), columns=self.df[variable], index = self.df['x'][0])
        return dt
