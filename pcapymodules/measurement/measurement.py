...
class WritePort:
    ...
    def __init__(self, name, unit,  measure_function, set_function, min_limit = None, max_limit = None, rate = 0.0, step=0.0, precision = 0.0):
        self.name = name
        self.unit = unit
    ...
    def get_info(self):
        print ('Name: {}, Unit: {}, \nPresent level: {}, Rate: {}/min, Step: {}, \nMin: {}, Max: {}, \nPrecision: {}, Timeout: {}'
               .format(self.name, self.unit, self.measure(), self.rate, self.step, self.min_limit, self.max_limit, self.precision, self.timeout))
    
    def check_parameter(self, full=True):
        comment = ''
        if self.min_limit == None: 
            comment += '\nMin is not set!'
        elif self.max_limit == None: 
            comment += '\nMax is not set!'
        elif self.min_limit >= self.max_limit: comment += '\nCheck min amd max are set correctly!'
    ...
    def ramp_to(self, target, present = None):
        ...
        if self.check_parameter():
            ...
            if self.min_limit <= target <= self.max_limit:
...
class TestInstrument:
    def __init__(self, name):
         self.name = name  
         print ('Connection to instrument:{} established.'.format(self.name))
